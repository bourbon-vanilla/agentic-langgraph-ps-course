...
class StrengthsOutput(BaseModel):
    strengths: str = Field(..., description="Candidate's key strengths relevant to the role")
    candidate_score: float = Field(..., ge=0, le=1, description="Score between 0 and 1 indicating strength level")
...
def extract_strengths(state: ScreeningState) :
    ...
    """)
    return {"strengths": response.strengths, "candidate_score": response.candidate_score }
...
def extract_weaknesses(state: ScreeningState):
    ...
    """)
    return {"weaknesses": response.weaknesses,"candidate_score": response.candidate_score}
