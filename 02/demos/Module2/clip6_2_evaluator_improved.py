...
def post_generator(state: State):
    ...
    if state.get("feedback") and state.get("post"):
        ...
        prompt = f"Write an engaging, professional LinkedIn post about '{state['topic']}'."
...
def post_evaluator(state: State):
    ...
    return {"quality": review.verdict, "feedback": review.feedback, "attempts": state.get("attempts", 0) + 1 }
...
with open("evaluator_graph_safe.png", "wb") as f:
